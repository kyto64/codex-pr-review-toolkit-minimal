...
[Apache License 2.0](./LICENSE)。本リポジトリ全体（派生 Skill・生成物・独自コンポーネントを含む）は Apache-2.0 に統一しています。上記 6 観点 Skill は [anthropics/claude-plugins-official](https://github.com/anthropics/claude-plugins-official) の `pr-review-toolkit` を由来とする派生作品です。upstream に `NOTICE` はなく、本リポジトリの `LICENSE` は upstream plugin LICENSE と同一の Apache-2.0 全文です。帰属の区分は [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md) を参照してください。
...
リリース履歴は GitHub Releases で管理します。初回公開版は `v0.1.0` です。
